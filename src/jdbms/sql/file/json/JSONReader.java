...
public class JSONReader implements FileReader {
	...
	public Table parse(final String tableName, final String databaseName, final String path)
		...
		final File jsonFile = new File(path
				...
				+ tableName.toUpperCase() + JSON_EXTENSION);
	...
	private class SQLTypeDeserializer implements JsonDeserializer<SQLType<?>> {
		private static final String VALUE_FIELD = "value";
		@Override
		public SQLType<?> deserialize(final JsonElement json,
				final Type type,
				final JsonDeserializationContext jsonDeserializationContext)
						throws JsonParseException {
			final DataTypesValidator validator = new DataTypesValidator();
			final JsonElement value = json.getAsJsonObject().get(VALUE_FIELD);
			final String dataType = validator.getDataType(value.getAsString());
			if (dataType == null) {
				System.out.println(value.getAsString());
			}
			try {
				return SQLTypeFactory.getInstance().getTypeObject(dataType,
						value.getAsString());
	...
	implements JsonDeserializer<TableColumn> {
		...
		public TableColumn deserialize(final JsonElement json,
			...
			for (final JsonElement sqlObject : values) {
				...
				if (value.equals(NULL_INDICATOR)) {
					value = null;
				}
				try {
					column.add(value);
