...
public class ResultSetTesting {
    ...
    @Test
    public void testGettingDataByIndex()
            throws SQLException,
            ParseException {
        final DataResultSet resultSet
                = new DataResultSet(DBStatement);
    ...
    @Test
    public void testGettingDataByName()
            throws SQLException,
            ParseException {
        final DataResultSet resultSet
                = new DataResultSet(DBStatement);
        ...
        final int id
                = resultSet.getInt("ID");
        assertEquals(id, 1);
        final String customer = resultSet.
                getString("Customer");
    ...
    private void constructFourRowedTable(
            final DataResultSet resultSet) {
        ...
        columns.add("Balance");
        final ArrayList<String> row1
                = new ArrayList<>();
        row1.add("1");
        row1.add("Ahmed Mohamed");
        row1.add("2009-09-09");
        row1.add("9999.99999");
        final ArrayList<String> row2
                = new ArrayList<>();
        row1.add("2");
        row1.add("Mohamed Ahmed");
        row1.add("2010-10-10");
        row1.add("12345.99999");
        final ArrayList<String> row3
                = new ArrayList<>();
        row1.add("3");
        row1.add("Ahmed Adam");
        row1.add("2011-11-11");
        row1.add("100.0");
        final ArrayList<String> row4
                = new ArrayList<>();
        row1.add("1");
        row1.add("Mohamed Adam");
        row1.add("2012-12-12");
        row1.add("0.0");
        final ArrayList<ArrayList<String>> columnRows
                = new ArrayList<>();
        columnRows.add(row1);
        columnRows.add(row2);
        columnRows.add(row3);
        ...
        final Map<String, Integer> columnTypes
                = new HashMap<>();
        columnTypes.put(columns.get(0).
                toUpperCase(), Types.INTEGER);
        columnTypes.put(columns.get(1).
                toUpperCase(), Types.VARCHAR);
        columnTypes.put(columns.get(2).
                toUpperCase(), Types.DATE);
        columnTypes.put(columns.get(3).
                toUpperCase(), Types.DOUBLE);
    ...
    private void constructThreeRowedTable(
            final DataResultSet resultSet) {
        final String tableName
                = "Students";
        final ArrayList<String> columns
