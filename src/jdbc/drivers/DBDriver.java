...
public class DBDriver implements Driver {
	...
	public Connection connect(final String url,
		...
		if (!isBelowMaxNoOfConnections()) {
			...
			= new SQLException("Connections"
					+ " Overloaded");
			logger.error("Current number"
					+ " of connections exceeded "
					+ CONNECTIONS_MAX, ex);
		}
		noOfConnections++;
	...
	private boolean isValidURL(final String url) {
		final ProtocolConstants constants
		= new ProtocolConstants();
		if (!url.startsWith(constants.
				getUrlPrefix())
				|| !url.endsWith(constants.
						getUrlSuffix())) {
			return false;
		} else {
			final String protocol = url.
					substring(
							url.indexOf(constants.
									getSeparator()) + 1,
							url.lastIndexOf(constants.
									getSeparator()));
			return isValidProtocol(protocol);
		}
	}

	private boolean isValidProtocol(final
			String protocol) {
		final ProtocolConstants constants
		= new ProtocolConstants();
